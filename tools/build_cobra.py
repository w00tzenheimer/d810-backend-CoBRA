...
# Anchor on the submodule rather than a fixed number of parents. This script
# came from d810, where it sat at tools/scripts/ and parents[2] was the repo
# root; here it sits at tools/ and that index would resolve one level too high,
# silently pointing DEFAULT_ROOT outside the repo.
def _repo_root() -> Path:
    for candidate in Path(__file__).resolve().parents:
        if (candidate / "third_party" / "cobra").is_dir():
            return candidate
    # Fall back to the layout this file ships in: <repo>/tools/build_cobra.py
